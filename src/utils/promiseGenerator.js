...
promiseAllTasks
	.then((r) => {
		console.log(
			'####### promiseGenerator() > .then > 1 resolvedSeconds: ',
			r[0],
			' SecondsElapsed: ',
			timeElapsedModule1.getSecondsElapsed(),
		);
		console.log(
			'####### promiseGenerator() > .then > 3 resolvedSeconds: ',
			r[1],
			' SecondsElapsed: ',
			timeElapsedModule1.getSecondsElapsed(),
		);
	...
	.catch(function (err) {
		console.log('####### promiseGenerator() > promiseAllTasks > .catch: ', err);
	});
